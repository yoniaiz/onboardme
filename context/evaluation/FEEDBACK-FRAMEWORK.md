...
Game questions are **generated by AI** based on the target codebase analysis. This means:
- Question difficulty varies depending on the codebase complexity and AI interpretation
- Questions may range from straightforward to challenging based on available code patterns
- The AI attempts to create meaningful questions, but quality depends on input data

### Testing Phase Focus

This evaluation is conducted during **active development and testing**. Key considerations:
- **Primary focus is on experience**, not question difficulty
- Questions may not represent final production quality or optimal challenge levels
- Content generation is being refined — evaluate the *framework and UX*, not content depth
- Report issues with question quality separately from experience feedback

### What to Prioritize

When evaluating, focus on:
1. **Flow and interaction** — Does the game feel smooth to play?
2. **Visual presentation** — Is the UI clear and appealing?
3. **Feedback and clarity** — Do you understand what's happening?
4. **Controls and navigation** — Is it intuitive to use?

Lower priority for now:
- Whether questions are "too easy" or "too hard"
...
> **Note:** Questions are AI-generated and may vary in difficulty. During testing, focus on whether the *game mechanics* handle different difficulty levels well, not whether the specific questions are challenging enough.

| ID | Statement |
|----|-----------|
| CH-1 | The game mechanics could accommodate various difficulty levels |
| CH-2 | The pacing between questions felt appropriate |
| CH-3 | The game provided fair feedback regardless of question difficulty |
...
> **Note:** Focus on whether the game *rewards progress* effectively, independent of AI-generated question difficulty.

| ID | Statement |
|----|-----------|
| MA-1 | The game made me feel accomplished when progressing |
| MA-2 | Correct answers felt rewarding |
| MA-3 | I felt capable while navigating the game interface |
...
### AI Question Quality (Separate from Experience)

This section captures feedback on AI-generated questions, reported separately from the experience evaluation.

```yaml
question_quality:
  # Overall impression of generated questions
  overall_impression: "appropriate" | "too_easy" | "too_hard" | "inconsistent" | "unclear"
  
  # Specific question issues (optional)
  question_issues:
    - question_text: "string"
      issue_type: "too_easy" | "too_hard" | "ambiguous" | "incorrect" | "irrelevant"
      notes: "string"
  
  # General notes on question generation
  notes: "string"
```

> **Important:** Question quality feedback informs AI generation improvements but should not heavily influence experience scores. The experience evaluation focuses on *how the game presents and handles* questions, not the questions themselves.
